...
rm -rf ~/ah/nvidia-settings
cp -r nvidia-settings ~/ah/nvidia-settings
cp ~/Downloads/NVIDIA-Linux-x86_64-570.133.07.run ~/ah/nvidia-settings

# Move to build directory
cd ~/ah/nvidia-settings || exit 1

# Build and sign package
makepkg -sr --sign || exit 1

# Remove old package and signature from the repo
rm -f /mnt/nvme/SteavenRepo/x86_64/nvidia-settings*.pkg.tar.zst
rm -f /mnt/nvme/SteavenRepo/x86_64/nvidia-settings*.pkg.tar.zst.sig
