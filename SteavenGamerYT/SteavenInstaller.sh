...
rm -rf ~/ah/SteavenInstaller
cp -r SteavenInstaller ~/ah/SteavenInstaller

# Move to build directory
cd ~/ah/SteavenInstaller || exit 1

# Build and sign package
makepkg -sr --sign || exit 1

# Remove old package and signature from the repo
rm -f /mnt/hdd/SteavenRepo/x86_64/SteavenInstaller*.pkg.tar.zst
rm -f /mnt/hdd/SteavenRepo/x86_64/SteavenInstaller*.pkg.tar.zst.sig
