...
rm -rf ~/ah/davinci-vaapi-encoder-plugin
cp -r davinci-vaapi-encoder-plugin ~/ah/davinci-vaapi-encoder-plugin

# Move to build directory
cd ~/ah/davinci-vaapi-encoder-plugin || exit 1

# Build and sign package
makepkg -sr --sign || exit 1

# Remove old package and signature from the repo
rm -f /mnt/nvme/SteavenRepo/x86_64/davinci-vaapi-encoder-plugin*.pkg.tar.zst
rm -f /mnt/nvme/SteavenRepo/x86_64/davinci-vaapi-encoder-plugin*.pkg.tar.zst.sig
