...
rm -rf ~/ah/nvidia-utils
cp -r nvidia-utils ~/ah/nvidia-utils
cp ~/Downloads/NVIDIA-Linux-x86_64-570.133.07.run ~/ah/nvidia-utils

# Move to build directory
cd ~/ah/nvidia-utils || exit 1

# Build and sign package
makepkg -sr --sign || exit 1

# Remove old package and signature from the repo
rm -f /mnt/nvme/SteavenRepo/x86_64/nvidia-utils*.pkg.tar.zst
rm -f /mnt/nvme/SteavenRepo/x86_64/nvidia-utils*.pkg.tar.zst.sig
