...
rm -rf ~/ah/dxvk-bin
cp -r dxvk-bin ~/ah/dxvk-bin

# Move to build directory
cd ~/ah/dxvk-bin || exit 1

# Build and sign package
makepkg -sr --sign || exit 1

# Remove old package and signature from the repo
rm -f /mnt/nvme/SteavenRepo/x86_64/dxvk-bin*.pkg.tar.zst
rm -f /mnt/nvme/SteavenRepo/x86_64/dxvk-bin*.pkg.tar.zst.sig
