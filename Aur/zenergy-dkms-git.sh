...
rm -rf ~/ah/zenergy-dkms-git
cp -r zenergy-dkms-git ~/ah/zenergy-dkms-git

# Move to build directory
cd ~/ah/zenergy-dkms-git || exit 1

# Build and sign package
makepkg -sr --sign || exit 1

# Remove old package and signature from the repo
rm -f /mnt/nvme/SteavenRepo/x86_64/zenergy-dkms-git*.pkg.tar.zst
rm -f /mnt/nvme/SteavenRepo/x86_64/zenergy-dkms-git*.pkg.tar.zst.sig
