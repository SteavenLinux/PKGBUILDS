...
rm -rf ~/ah/openrgb-plugin-http-hook-git
cp -r openrgb-plugin-http-hook-git ~/ah/openrgb-plugin-http-hook-git

# Move to build directory
cd ~/ah/openrgb-plugin-http-hook-git || exit 1

# Build and sign package
makepkg -sr --sign || exit 1

# Remove old package and signature from the repo
rm -f /mnt/Omar-PC-Server/home/omarhanykasban/SteavenRepo/x86_64/openrgb-plugin-http-hook-git*.pkg.tar.zst
rm -f /mnt/Omar-PC-Server/home/omarhanykasban/SteavenRepo/x86_64/openrgb-plugin-http-hook-git*.pkg.tar.zst.sig
